...
```js
function find(rats) {
  // Cuando hacemos la calculacion de la potencia, la base es 2 y el exponente es el numero de rata
  return rats.reduce((prev, curr) => prev + Math.pow(2, curr),0);
}

// Probando (find([0, 1, 2]), 7);
// 1. prev = 0 + current(la potencia de 2 a la 0 = 1) = 1
// 2. prev = 1 + current(la potencia de 2 a la 1 = 2) = 3
// 2. prev = 3 + current(la potencia de 2 a la 2 = 4) = 7

// Probando (find([3, 5, 6, 7, 8, 9]), 1000);
// 1. prev = 0 + current(la potencia de 2 a la 3 = 8) = 8 
// 2. prev = 8 + current(la potencia de 2 a la 5 = 32) = 40
// 3. prev = 40 + current(la potencia de 2 a la 6 = 64) = 104
// 4. prev = 104 + current(la potencia de 2 a la 7 = 128) = 232
// 5. prev = 232 + current(la potencia de 2 a la 8 = 256) = 488
// 6. prev = 488 + current(la potencia de 2 a la 9 = 512) = 1000
```
...
function arrayDiff(a, b) {
  return a.filter((eachElementArray1) => !b.includes(eachElementArray1));
}

// Con el filter reviso cada elemento en el array a, ya que la callback funcion de filter
// devuelve un true o false, hago una busqueda con la funcion includes para que busque 
// si el elemento que estoy buscando esta dentro del segundo array, si lo encuentra seria 
// verdadero pero en el filter debe ser alreves, asi que por eso utilizo el operador ! para 
// que si me da verdadero sea falso
```
